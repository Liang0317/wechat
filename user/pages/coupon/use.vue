		...
		<view @tap.stop="toUse(index)" class="list-item rel mt-md ml-lg mr-lg fill-base radius-16"
			...
			<view class="pl-lg pr-lg pb-lg fill-space f-desc c-caption" v-if="item.is_show">
				...
				<view class="flex-warp mt-md">
					<view>限用服务：</view>
					<view class="flex-1">
						<block v-if="item.send_type==3">所有服务</block>
						<block v-else>
							<view :class="[{'mt-sm':aindex!=0}]" v-for="(aitem,aindex) in item.service" :key="aindex">
								{{aitem.title}}
							</view>
							<view v-if="item.service.length==0">-</view>
						</block>
					</view>
				</view>
				<view class="mt-md c-warning">{{item.admin_id?'仅限部分城市可使用':'通用券'}}</view>
			</view>
		</view>
