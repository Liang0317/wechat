<template>
	<view class="order-pages" v-if="isLoad">
					...
					<view class="flex-1 ml-md">
						...
						<view class="flex-center">
							<view class="flex-1 flex-y-center f-desc c-caption">
								...
								<!-- <view class="text-delete mr-sm" v-if="bellOrderParams.order_type ==0">¥{{item.price}}
								</view> -->
								{{`/ ${item.time_long}`}}分钟
							</view>
						...
						<view class="flex-between">
							...
							<view>
								<view class="flex-warp" v-if="bellOrderParams.order_type==1">
		...
		<abnor @confirm="$util.goUrl({url:1,openType:`navigateBack`})" :tip="[{ text: '该服务已下架~', color: 0 }]"
			:button="[{ text: '去看看其他加钟服务' , type: 'confirm' }]" btnSize="" v-else>
		</abnor>
					...
					if (pay_list) {
						...
						if (pay_model == 3) {
							pay_list = Object.assign({}, pay_list, {
								order_id,
								is_update: order_type == 0 ? 1 : 0,
								page_url: `/pages/order?tab=3`
							})
