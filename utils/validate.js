const strategies = {
	...
	isNumber: function(value, errorMsg, regType) {
		// 1可为0；2大于0
		let reg = regType == 1 ? /^\+?[0-9]*$/ : /^[1-9]\d*$/
		if (!reg.test(value * 1)) {
			let msg = regType == 2 ? `${errorMsg}，必须是大于0的整数` : `${errorMsg}，必须是整数`
			return msg
		}
	},
	isFloatNum: function(value, errorMsg, regType, dotLen, maxNum, unit) {
		// dotLen 保留几位小数
		// regType 1大于0；2可为0 
		let regArr = {
			1: {
		...
		if (value == '') {
			...
			return `${errorMsg}，不能大于${maxNum}${unit}`;
		}
	},
...
Validate.prototype.add = function(value, item = {}) {
	...
	this.cache.push(function() {
		...
		let regTypeArr = ['isNotNull', 'isAllPhone', 'isMoney', 'isNumber', 'isFloatNum']
		...
		let maxNumArr = ['isFloatNum']
		if (maxNumArr.includes(checkType)) {
			arr.push(maxNum);
		}
		let unitArr = ['isFloatNum']
